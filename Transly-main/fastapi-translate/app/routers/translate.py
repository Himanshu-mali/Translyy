...
async def translate_text(payload: TranslateRequest):
    ...
    try:
        source_lang = payload.source_lang or "ne"
        target_lang = getattr(payload, "target_lang", "en") or "en"
        translated = translate_with_model(state.model_bundle, text, source_lang, target_lang)
